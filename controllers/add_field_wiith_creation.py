...
class AuthSignupHome(Home):
    ...
    def do_signup(self, qcontext):
        ...
        values = {key: qcontext.get(key) for key in ('login', 'name', 'password', 'phone', 'mobile')}
        values.update({'phone': int(qcontext.get('phone'))})
        values.update({'mobile': int(qcontext.get('mobile'))})
        print(values)
        if not values:
